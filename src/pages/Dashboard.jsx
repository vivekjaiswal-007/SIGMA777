...
export default function Dashboard() {
  ...
  const generateQRCodes = async () => {
    ...
    try {
      ...
      const upiString = `upi://pay?pa=sigma777@upi&pn=SIGMA777&am=${depositAmount}&cu=INR&tn=RB${nonce}`
      setQrCodes([{ upiId: 'sigma777@upi', name: 'SIGMA777 Casino', amount: depositAmount, coins: depositAmount, txnRef: `RB${nonce}`, qrUrl: `https://api.qrserver.com/v1/create-qr-code/?size=250x250&data=${encodeURIComponent(upiString)}&margin=10&_nc=${nonce}` }])
      setSelectedQR(0)
    }
    setQrLoading(false)
  }
  ...
  return (
    <div style={{ maxWidth: '1100px', margin: '0 auto' }}>
      ...
      {/* Profile bar */}
      <div style={{ background: 'linear-gradient(135deg,rgba(224,48,48,0.08),rgba(153,68,255,0.06))', border: '1px solid rgba(224,48,48,0.2)', borderRadius: '14px', padding: '14px 18px', marginBottom: '20px', display: 'flex', gap: '12px', alignItems: 'center', flexWrap: 'wrap' }}>
        ...
        <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
          <div style={{ padding: '6px 14px', borderRadius: '20px', background: 'rgba(224,48,48,0.12)', border: '1px solid rgba(224,48,48,0.3)', color: 'var(--gold)', fontWeight: '700', fontSize: '14px' }}>🪙 {balance.toLocaleString()}</div>
      ...
      {tab === 'overview' && (
        <div style={{ animation: 'fadeUp 0.3s ease' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(140px,1fr))', gap: '10px', marginBottom: '18px' }}>
            <Stat label="Balance" value={`🪙 ${balance.toLocaleString()}`} color="#e03030" icon="💰" />
            ...
            <Stat label="Best Win" value={`+${biggestWin.toLocaleString()} 🪙`} color="#e03030" icon="🏆" />
          </div>
        ...
        <div>
          ...
          <div style={{ display: 'flex', gap: '14px', padding: '10px 14px', background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '10px', marginBottom: '12px', flexWrap: 'wrap' }}>
            {[
              ...
              { l: 'Wagered', v: `🪙${totalWagered.toLocaleString()}`, c: '#e03030' },
                  ...
                  {loading ? (
                    ...
                    return (
                      <tr key={i} style={{ borderBottom: '1px solid rgba(255,255,255,0.025)' }}>
                        ...
                        <td style={{ padding: '10px 12px', fontSize: '12px', color: '#e03030', fontWeight: '700', whiteSpace: 'nowrap' }}>🪙 {bet.betAmount?.toLocaleString()}</td>
      ...
      {tab === 'deposit' && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(270px,1fr))', gap: '16px' }}>
          <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '12px', padding: '18px' }}>
            ...
            <div style={{ padding: '9px 12px', background: 'rgba(224,48,48,0.07)', border: '1px solid rgba(224,48,48,0.18)', borderRadius: '8px', marginBottom: '14px', fontSize: '12px' }}>
              <span style={{ color: '#e03030', fontWeight: '700' }}>₹1 = 🪙 1 coin</span>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '7px', marginBottom: '12px' }}>
              {[100,200,500,1000,2000,5000].map(amt => (
                <button key={amt} onClick={() => { setDepositAmount(amt); setQrCodes([]); setSelectedQR(null) }} style={{
                  ...
                  background: depositAmount === amt ? 'rgba(224,48,48,0.2)' : 'var(--bg-hover)',
            ...
            <button onClick={generateQRCodes} disabled={qrLoading || depositAmount < 100} style={{
              ...
              background: qrLoading ? 'rgba(224,48,48,0.25)' : 'linear-gradient(135deg,#e03030,#ff5555)',
              ...
              <>
                ...
                {selectedQR !== null && qrCodes[selectedQR] && (()=>{const qr=qrCodes[selectedQR];return(
                    <div style={{ textAlign: 'center' }}>
                      <div style={{ display: 'inline-block', padding: '12px', background: 'white', borderRadius: '10px', border: '3px solid var(--gold)', boxShadow: '0 0 20px rgba(224,48,48,0.25)', marginBottom: '10px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(270px,1fr))', gap: '16px' }}>
          ...
          <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '12px', overflow: 'hidden' }}>
            ...
            {withdrawHistory.length === 0 ? (
              ...
              const sc = wd.withdrawStatus === 'approved' ? '#00d084' : wd.withdrawStatus === 'rejected' ? '#ff4444' : '#e03030'
              return (
