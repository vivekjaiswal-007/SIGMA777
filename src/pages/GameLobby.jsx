      ...
      <div style={{ display: 'flex', gap: '5px', marginBottom: '18px', overflowX: 'auto', WebkitOverflowScrolling: 'touch', paddingBottom: '4px' }}>
        {CATEGORIES.map(c => (
          <button key={c.id} onClick={() => setCat(c.id)}
            style={{ padding: '8px 14px', borderRadius: '20px', border: `1px solid ${cat === c.id ? 'var(--gold)' : 'var(--border)'}`, background: cat === c.id ? 'rgba(224,48,48,0.15)' : 'var(--bg-card)', color: cat === c.id ? 'var(--gold)' : 'var(--text-secondary)', fontWeight: cat === c.id ? '700' : '400', fontSize: '12px', cursor: 'pointer', whiteSpace: 'nowrap', flexShrink: 0, transition: 'all 0.2s' }}>
            {c.icon} {c.label}
            ...
            <div key={group.id}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '12px' }}>
                ...
                <button onClick={() => setCat(group.id)}
                  style={{ padding: '4px 12px', borderRadius: '6px', background: 'rgba(224,48,48,0.1)', border: '1px solid rgba(224,48,48,0.2)', color: 'var(--gold)', fontSize: '11px', fontWeight: '600', cursor: 'pointer' }}>
                  See All →
                </button>
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(120px, 1fr))', gap: '10px' }}>
