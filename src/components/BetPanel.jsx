  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        padding: '9px 13px',
        background: 'rgba(224,48,48,0.07)', borderRadius: '8px',
      ...
      <div style={{ display: 'flex', gap: '6px' }}>
        {['Manual', 'Auto'].map(tab => (
          <button key={tab} onClick={() => { setAutoMode(tab === 'Auto'); stopAuto() }} style={{
            ...
            background: (tab === 'Auto') === autoMode ? 'rgba(224,48,48,0.2)' : 'var(--bg-hover)',
      ...
      <div className="quick-amounts" style={{ display: 'flex', gap: '5px', flexWrap: 'wrap' }}>
        {quickAmounts.map(a => (
          <button key={a} onClick={() => setBetAmount(a)} style={{
            ...
            background: betAmount === a ? 'rgba(224,48,48,0.2)' : 'var(--bg-hover)',
          ...
          <div style={{ display: 'flex', gap: '8px' }}>
            {[{ label: 'Stop on Win', val: stopOnWin, set: setStopOnWin }, { label: 'Stop on Loss', val: stopOnLoss, set: setStopOnLoss }].map(({ label, val, set }) => (
              <button key={label} onClick={() => set(!val)} style={{
                ...
                background: val ? 'rgba(224,48,48,0.2)' : 'var(--bg-hover)',
      ...
      {(!autoMode || noAuto) ? (
        <button onClick={handleBet} disabled={!canBet} style={{
          ...
          background: !canBet ? 'rgba(224,48,48,0.25)' : 'linear-gradient(135deg,#e03030,#ff5555)',
