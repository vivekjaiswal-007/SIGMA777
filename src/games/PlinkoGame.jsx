  ...
  useEffect(()=>{
    ...
    const loop=()=>{
      ...
      ballsRef.current.forEach(ball=>{
        ...
        if(ball.trail)ball.trail.forEach((pt,ti)=>{ctx.beginPath();ctx.arc(pt.x,pt.y,BALL_R*(ti/ball.trail.length)*0.5,0,Math.PI*2);ctx.fillStyle=`rgba(224,48,48,${(ti/ball.trail.length)*0.25})`;ctx.fill()})
        ctx.beginPath();ctx.arc(ball.x,ball.y,BALL_R,0,Math.PI*2);const bg2=ctx.createRadialGradient(ball.x-2,ball.y-2,1,ball.x,ball.y,BALL_R);bg2.addColorStop(0,'#ff5555');bg2.addColorStop(0.6,'#e03030');bg2.addColorStop(1,'#8a6010');ctx.fillStyle=bg2;ctx.shadowBlur=10;ctx.shadowColor='#e03030';ctx.fill();ctx.shadowBlur=0
      })
    ...
    <div style={{ maxWidth:'900px',margin:'0 auto' }}>
      ...
      <div className="game-layout">
        ...
        <div className="game-panel-right">
          <BetPanel onBet={drop} />
          {active>0&&<div style={{ padding:'10px',background:'rgba(224,48,48,0.1)',border:'1px solid rgba(224,48,48,0.3)',borderRadius:'8px',textAlign:'center',color:'var(--gold)',fontSize:'13px' }}>🎯 {active} ball{active>1?'s':''} in play</div>}
          <div style={{ background:'var(--bg-card)',border:'1px solid var(--border)',borderRadius:'12px',padding:'13px' }}>
            ...
            <div style={{ display:'flex',flexWrap:'wrap',gap:'3px' }}>
              {MULTS.map((m,i)=>(
                <span key={i} style={{ padding:'2px 6px',borderRadius:'4px',fontSize:'11px',fontWeight:'700',background:m===0?'rgba(60,60,80,0.4)':m>=41?'rgba(224,48,48,0.22)':m>=10?'rgba(255,68,136,0.18)':m>=3?'rgba(0,208,132,0.14)':'rgba(68,136,255,0.14)',color:m===0?'#444455':m>=41?'#e03030':m>=10?'#ff4488':m>=3?'#00d084':'#4488ff' }}>{m}x</span>
              ))}
            </div>
